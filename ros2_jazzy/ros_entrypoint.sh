...
# The base image builds ROS from source with `colcon build --merge-install` into
# $ROS_ROOT, so the setup script stays under install/ on jazzy just like on humble.
